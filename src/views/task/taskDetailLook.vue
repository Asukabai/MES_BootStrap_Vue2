    ...
    getPersonName(value) {
      ...
      try {
        ...
        if (Array.isArray(obj)) {
          // 提取数组中所有对象的 Person_Name 属性值，并过滤空值
          const names = obj.map(item => {
            return item && item.Person_Name ? item.Person_Name : '';
          }).filter(name => name !== ''); // 过滤掉空字符串

          return names.length > 0 ? names.join(', ') : '暂无数据';
        }

        // 如果不是数组，直接返回单个对象的 Person_Name
        return obj && obj.Person_Name ? obj.Person_Name : '暂无数据';
      } catch (e) {
