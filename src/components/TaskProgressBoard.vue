  ...
  <div class="progress-tracking-container">
    <div class="content">
      ...
      <div class="board-section">
        ...
        <!-- 卡片内容区域 -->
        <van-row gutter="10">
          <van-col :span="isPC ? 12 : 24" v-for="(item, index) in displayedTaskProgressData" :key="index">
            <div class="card">
              <p>{{ item.name }}</p>
        ...
        <!-- 底部提示信息 -->
        <div class="bottom-tip" v-if="!isExpanded && taskProgressData.length > 6">
          到底了，默认展示6条数据
        </div>
        <div class="bottom-tip" v-else-if="isExpanded && taskProgressData.length > 0">
          到底了，已加载所有数据
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    displayedTaskProgressData() {
      return this.isExpanded ? this.taskProgressData : this.taskProgressData.slice(0, 6);
    },
    shouldShowViewMore() {
      return this.taskProgressData.length > 6;
    }
  },
  methods: {
    checkIsPC() {
      return window.innerWidth > 768; // 或其他你认为合适的断点值
    },
    toggleExpand() {
